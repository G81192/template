...
from app.models.framework.Scheduler import DatabaseSchedulerEntry, CrontabSchedule, IntervalSchedule, SchedulerHistory
...
class Entry(ScheduleEntry):
    ...
    def __init__(self, model):
        ...
        if not model.last_run_at:
            ...
            model.last_run_at = model.date_changed if model.date_changed else self._default_now()
    ...
    def is_due(self):
        ...
        db.session.refresh(self.model)  # 解决多实例发布多个定时任务问题
        if not self.model.last_run_at:
            # self.model.last_run_at = self._default_now()
            self.model.last_run_at = self.model.date_changed if self.model.date_changed else self._default_now()
